...
static int tuner_open(struct inode *inode, struct file *file)
{
	...
	if (ret == 1) {
		/* Failed */
		printk("!!!tuner_open gpio_init() error \n");
		return (-1);
	}
...
static long tuner_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (copy_from_user(io_cmd, (ioctl_cmd*)arg, sizeof(ioctl_cmd))) {
		printk("!!!tuner_ioctl copy_from_user error %08x\n", (unsigned int)arg);
		return -EFAULT;
	}
	...
	case IOC_GPIO_VAL_SET:
		...
		if (ret != 0) {
			printk("!!!tuner_ioctl set error[cmd %x val %d][ret:%d]\n", io_cmd->no, io_cmd->val, ret);
			return -EINVAL;
		}
		break;
	case IOC_GPIO_VAL_GET:
		...
		if (ret != 0) {
			printk("!!!tuner_ioctl get error[cmd %x val %d][ret:%d]\n", io_cmd->no, io_cmd->val, ret);
			return -EINVAL;
		}
		if (copy_to_user((ioctl_cmd*)arg, io_cmd, sizeof(ioctl_cmd))) {
			printk("!!!tuner_ioctl copy_to_user error %08x\n", (unsigned int)arg);
			return -EFAULT;
		}
		break;
	case IOC_VREG_ENABLE:
		...
		if (ret != 0) {
			printk("!!!tuner_ioctl set error[ret:%d]\n", ret);
			return -EINVAL;
		}
		break;
...
static int gpio_init(void)
{
	...
	for (i=0; i<loop; i++, p++) {
		if (p->direction == DirctionIn) {
			...
			if (gpio_direction_input(p->no) < 0) {
				/* Failed */
				errcnt ++;
				printk( "gpio_direction_input error NO.%d \n", p->no);
				continue;
			}
			p->init_done = 1;
			continue;
		...
		if (p->direction == DirctionOut) {
			...
			if (gpio_direction_output(p->no, p->out_val) < 0) {
				/* Failed */
				errcnt ++;
				printk("gpio_direction_output error NO.%d \n", p->no);
				continue;
			}
			p->init_done = 1;
			continue;
	...
	if (errcnt != 0) {
		printk("gpio_init error count %d\n", errcnt);
		return 1;
	}
	return 0;
}
...
static int gpio_set(unsigned int no, int value)
{
	...
	if (flag == 0) {
		printk("!!! gpio_set() error No.%d value %d \n", no, value);
		return EINVAL;
	}
