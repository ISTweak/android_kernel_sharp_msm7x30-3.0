...
/* [WLAN][SHARP] 2011.09.07 add for modify the NVRAM Start */
#include "sharp/sh_smem.h"
#define SH_SMEM_READ_LEN 20
#define SH_WLAN_CAL_ITEM_VALUE_SIZE 10
#define SH_LOG_LEN 80
#define SH_WLAN_CAL_ITEM_CONJUNCTION "=0x"

typedef	struct sh_wlan_cal_item {
		char *name;
		int size; /* amount of char in nvram.txt */
		int valid;
		char value[SH_WLAN_CAL_ITEM_VALUE_SIZE];
	} sh_wlan_cal_item_t;
/* [WLAN][SHARP] 2011.09.07 add for modify the NVRAM End */
...
/* [WLAN][SHARP] 2011.04.01 Mod for Static Link Start */
#ifdef SH_CONFIG_WLAN_STATIC 
int reject_ioctl = 0;
#endif
/* [WLAN][SHARP] 2011.04.01 Mod for Static Link End */
...
static void
sh_nvram_debug_log(char *Head_line, char *Value_start, char *log_buf )
{
	char *log_bufP = log_buf;
	char *Head_lineP = Head_line;
	char *Value_startP = Value_start-2;	//two-byte back for '0x'
	
	if( Head_line != NULL ){
		while( *Head_lineP != '=' ){
			sprintf(log_bufP++, "%c", *Head_lineP++);
		}
		sprintf( log_bufP, ": " );
		log_bufP+=2;
	}
	while( *Value_startP != 0x0D && *Value_startP != 0x0A){
			sprintf(log_bufP++, "%c", *Value_startP++);
		}
...
static void
sh_nvram_memmove(char **End_lineP, sh_wlan_cal_item_t *wlan_cal_item, int mod_size, uint nvram_len, char *memblockP)
{
	uint remainder_len;

	/* initialize */
	remainder_len = nvram_len - ( *End_lineP - memblockP );

	memmove( *End_lineP + mod_size, (void *)(*End_lineP), remainder_len );
/*
	*nvram_len += mod_size;
	*End_lineP += mod_size;
*/
...
static void
sh_cp_wlan_cal_item(sh_wlan_cal_item_t *wlan_cal_item, char *shdarea_WlanCalPAData)
{
	int shdarea_WlanCalPAData_pos;
	bool check_shdarea_WlanCalPAData;
	int wlan_cal_item_len;
	char shdarea_WlanCalPAData_Str[SH_SMEM_READ_LEN*2];
	int wlan_cal_item_total_size;
	sh_wlan_cal_item_t *wlan_cal_itemP;

	/* initialize */
	shdarea_WlanCalPAData_pos = 0;
	check_shdarea_WlanCalPAData = FALSE;
	wlan_cal_item_total_size = 0;
	wlan_cal_item_len = 0;
	wlan_cal_itemP = wlan_cal_item;
	memset(shdarea_WlanCalPAData_Str, '\0', SH_SMEM_READ_LEN*2);

	while( shdarea_WlanCalPAData_pos < SH_SMEM_READ_LEN ){
		sprintf( shdarea_WlanCalPAData_Str+(shdarea_WlanCalPAData_pos*2), "%02X", *(shdarea_WlanCalPAData+shdarea_WlanCalPAData_pos) );
		shdarea_WlanCalPAData_pos++;
	}

	wlan_cal_itemP = wlan_cal_item;
	shdarea_WlanCalPAData_pos = 0;

	while(wlan_cal_itemP->name != NULL){
		
		wlan_cal_item_total_size += wlan_cal_itemP->size;
		wlan_cal_item_len = 0;
		check_shdarea_WlanCalPAData = FALSE;
		while( (shdarea_WlanCalPAData_pos+wlan_cal_item_len) < wlan_cal_item_total_size ){
			if( shdarea_WlanCalPAData_Str[shdarea_WlanCalPAData_pos+wlan_cal_item_len] != 'F' ){
				check_shdarea_WlanCalPAData = TRUE;
				break;
			}
			wlan_cal_item_len++;
		}

		if( check_shdarea_WlanCalPAData == TRUE ){
			wlan_cal_item_len = 0;
			check_shdarea_WlanCalPAData = FALSE;
			while( (shdarea_WlanCalPAData_pos+wlan_cal_item_len) < wlan_cal_item_total_size ){
				if( shdarea_WlanCalPAData_Str[shdarea_WlanCalPAData_pos+wlan_cal_item_len] != '0' ){
					check_shdarea_WlanCalPAData = TRUE;
					break;
				}
				wlan_cal_item_len++;
		...
		if( check_shdarea_WlanCalPAData ==TRUE ){	
			strncpy( wlan_cal_itemP->value, (shdarea_WlanCalPAData_Str+shdarea_WlanCalPAData_pos), wlan_cal_itemP->size );
			wlan_cal_itemP->valid = 1;
		}
		
		shdarea_WlanCalPAData_pos += wlan_cal_itemP->size;
		wlan_cal_itemP++;
	}
	
	wlan_cal_itemP = wlan_cal_item;
	while(wlan_cal_itemP->name != NULL){
		DHD_INFO(("%s: wlan_cal_item[%d].value=[%s]\n",
						 __FUNCTION__, wlan_cal_itemP - wlan_cal_item, 
						 wlan_cal_itemP->value
						 ));
		wlan_cal_itemP++;
	 }
...
static bool
sh_modify_nvram_init( sh_wlan_cal_item_t *wlan_cal_item )
{
	...
	while(wlan_cal_itemP->name != NULL){
		memset( wlan_cal_itemP->value, '\0', SH_WLAN_CAL_ITEM_VALUE_SIZE);
		wlan_cal_itemP++;
	}
	
	/* get calibration data for BCM4330 from SMEM*/
	SMemCommAdrP = sh_smem_get_common_address( ); 
	if( SMemCommAdrP == NULL ){
		DHD_ERROR(("%s: could not get smem address by sh_smem_get_common_address().\n", __FUNCTION__));
		return ret;
	}

	/* all 'F' check */
	loop_cnt = 0;
	while( loop_cnt < SH_SMEM_READ_LEN ){
		if( SMemCommAdrP->shdarea_WlanCalPAData[loop_cnt++] != 0xFF ){
			smem_chk_flg = TRUE;
			break;
		}
	}
	if( smem_chk_flg == FALSE ){
		return ret;
	}
	...
	smem_chk_flg = FALSE;
	while( loop_cnt < SH_SMEM_READ_LEN ){
		if( SMemCommAdrP->shdarea_WlanCalPAData[loop_cnt++] != 0x00 ){
			smem_chk_flg = TRUE;
			break;
		}
	}
	...
	if( loop_cnt == SH_SMEM_READ_LEN ){
		DHD_ERROR(("%s : Change value is not set!\n", __FUNCTION__));
		return ret;
	}
	
	wlan_cal_itemP = wlan_cal_item;
	sh_cp_wlan_cal_item(wlan_cal_itemP, SMemCommAdrP->shdarea_WlanCalPAData);
...
static int
sh_modify_nvram_set_element(char **Head_lineP, char **End_lineP, char **memblock_valueP, sh_wlan_cal_item_t *wlan_cal_itemP )
{
	*memblock_valueP = *Head_lineP + strlen(wlan_cal_itemP->name) + strlen(SH_WLAN_CAL_ITEM_CONJUNCTION);
	*End_lineP = *memblock_valueP;

	/* CR or LF, search for end of line */
	while( *(*End_lineP) != 0x0D && *(*End_lineP) != 0x0A ){
		*End_lineP+=1;
	}
	
	return (*End_lineP)-(*memblock_valueP);

}

static uint
sh_modify_nvram_add_element(char *memblock, char *Head_lineP, sh_wlan_cal_item_t *wlan_cal_itemP)
{
	uint ret_len;
	char *End_lineP;
	...
	/* initialize */
	ret_len = 0;
	End_lineP = Head_lineP;
	
	if( ( memblock + MAX_NVRAMBUF_SIZE )
			 <= Head_lineP + ( strlen(wlan_cal_itemP->name)+strlen(SH_WLAN_CAL_ITEM_CONJUNCTION)+wlan_cal_itemP->size ) + 2 ) {
		return ret_len;
	}
	
	sprintf(End_lineP, "%s%s%s", wlan_cal_itemP->name, SH_WLAN_CAL_ITEM_CONJUNCTION, wlan_cal_itemP->value);
	
	End_lineP += strlen(Head_lineP);
	if( *(Head_lineP-2) == 0x0D ){
		*End_lineP = 0x0D;
		End_lineP++;
	} 
	...
	*End_lineP = 0x0A;
	...
	ret_len = End_lineP - Head_lineP;
	
	Head_lineP = End_lineP;
	
	return ret_len;
}

static uint
sh_modify_nvram(char *memblock, uint len, struct dhd_bus *bus)
{
	char *Head_lineP;
	char *End_lineP;
	char *memblock_valueP;
	char *log_buf;
	int wlan_cal_item_cnt;
	int org_item_size;
	int mod_item_size;
	int loop_cnt;
	uint ret_len;
    ...
    sh_wlan_cal_item_t wlan_cal_item[] = {
		{"pa0b0"	, 4, 0, "" }, 
		{"pa0b1"	, 4, 0, "" }, 
		{"pa0b2"	, 4, 0, "" }, 
		{"maxp2ga0"	, 2, 0, "" }, 
	...
	/* initialize, add item here if required */
	Head_lineP = memblock;
	End_lineP = Head_lineP;
	memblock_valueP = NULL;
	wlan_cal_item_cnt = 0;
	ret_len = len;
	org_item_size=0;
	mod_item_size=0;
	...
	if( (sh_modify_nvram_init(wlan_cal_item)) == FALSE ){
		return ret_len;
	}

	if (dhd_msg_level & DHD_INFO_VAL){
		log_buf = MALLOC(bus->dhd->osh, SH_LOG_LEN);
	}
	
    /* read line */
	while ( ( Head_lineP - memblock ) < ret_len ){
		
		wlan_cal_item_cnt = 0;
		
		while(wlan_cal_item[wlan_cal_item_cnt].name != NULL){
			if( wlan_cal_item[wlan_cal_item_cnt].valid &&
			     (strncmp((Head_lineP), wlan_cal_item[wlan_cal_item_cnt].name, strlen(wlan_cal_item[wlan_cal_item_cnt].name)) == 0) ){
				org_item_size = sh_modify_nvram_set_element(&Head_lineP, &End_lineP,
												 &memblock_valueP,  &wlan_cal_item[wlan_cal_item_cnt] );
				mod_item_size = wlan_cal_item[wlan_cal_item_cnt].size - org_item_size;
				/* Processing to outpu debug log */
				if ( log_buf != NULL ) {
					memset( log_buf, '\0', SH_LOG_LEN );
					sh_nvram_debug_log( Head_lineP, memblock_valueP, log_buf );
				}

				/* Travels the memory, if necessary */
				if( mod_item_size ){
					sh_nvram_memmove( &End_lineP, &(wlan_cal_item[wlan_cal_item_cnt]),
										 mod_item_size, ret_len, memblock);
					ret_len += mod_item_size;
					End_lineP += mod_item_size;
				}
				
				/* copied modify data */
				strncpy( memblock_valueP, wlan_cal_item[wlan_cal_item_cnt].value, wlan_cal_item[wlan_cal_item_cnt].size );
				
				/* Processing to output debug log */
				if ( log_buf != NULL ) {
					sh_nvram_debug_log( NULL, memblock_valueP, (log_buf+strlen(log_buf)) );
					DHD_INFO( ("%s", log_buf) );
				}

				Head_lineP = End_lineP;
				wlan_cal_item[wlan_cal_item_cnt].valid = 0;
		...
		/* LF, search for end of line */
		while( *(Head_lineP++) != 0x0A ) ;
	...
	while(wlan_cal_item[wlan_cal_item_cnt].name != NULL){
		if( wlan_cal_item[wlan_cal_item_cnt].valid != 0 ){

			char *log_startP = Head_lineP;
			ret_len += sh_modify_nvram_add_element(memblock, Head_lineP, &wlan_cal_item[wlan_cal_item_cnt]);
			DHD_INFO(("%s: len+=[%d], Add to NVRAM [%s]\n", __FUNCTION__,
							 strlen(wlan_cal_item[wlan_cal_item_cnt].name)+ 3 + wlan_cal_item[wlan_cal_item_cnt].size + add_str_len,
							 log_startP
							 ));
	...
	if ( log_buf != NULL ) {
		MFREE(bus->dhd->osh, log_buf , SH_LOG_LEN );
	}
...
dhdsdio_download_nvram(struct dhd_bus *bus)
{
	...
	/* Download variables */
	if (nvram_file_exists) {
/* [WLAN][SHARP] 2011.09.07 mod  for modify the NVRAM Start */
#if 0
		len = dhd_os_get_image_block(memblock, MAX_NVRAMBUF_SIZE, image);
#else
		memset(memblock, 0, MAX_NVRAMBUF_SIZE );
		len = dhd_os_get_image_block(memblock, MAX_NVRAMBUF_SIZE, image);
		len = sh_modify_nvram(memblock, len, bus);
#endif
/* [WLAN][SHARP] 2011.09.07 mod  for modify the NVRAM End */
	}
	else {
